...
const Step3 = () => {
  ...
  const { nextStep, adminInfo } = useAuthContext();
  ...
  const handlePaste = (event) => {
    const pastedData = event.clipboardData.getData("Text").slice(0, 6); // Get the first 6 characters
    const newCode = pastedData.split("");
    setCode([...newCode, ...Array(6 - newCode.length).fill("")]);

    const lastField = inputRefs.current[Math.min(newCode.length, 5)];
  ...
  const ResendOTP = async () => {
    try {
      ...
      if (response.status === 201) {
        ...
        const result = await response.json();
        openToast(result.message || "Failed to resend OTP.", "error");
      }
    } catch (error) {
      openToast("Error while resending OTP: " + error.message, "error");
    }
  };
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4">
      ...
      <div className="bg-white rounded-lg shadow-lg p-8 w-full max-w-md">
        ...
        <p className="text-center text-sm">
          ...
          <a href="#" className="text-blue-500 hover:underline" onClick={ResendOTP}>
            Send it again
          </a>
        </p>
      </div>
