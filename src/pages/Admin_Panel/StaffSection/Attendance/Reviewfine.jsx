...
const Reviewfine = () => {
  ...
  function updateStaffFinePerMinuteState(
    ...
    const updatedStaffFine = staffList.map((staff) => {
      const salaryDetail = filterSalaryByMonthAndYear(
        staff?.staffDetails?.SalaryDetails,
        workMonth,
        workYear
      );
      ...
      return {
        staffId: staff?.staffDetails?.id,
        finePerMinute: calculateFinePerMinute(
          salaryDetail?.ctc_amount,
          workingDays,
          workingHoursPerDay
        ),
      };
  ...
  async function fetchFineDetails() {
    try {
      const result = await fetch(
        baseUrl +
          `fine?date=${workTimeDate.year}-${workTimeDate.month}-${workTimeDate.date}`
      );
      if (result.status == 200) {
  ...
  async function updateFine(fine) {
    try {
      const {
        ...
        lateEntryFineAmount: prevLateEntryFineAmount = 0,
      } =
        fineDetail?.find(
      ...
      const result = await fetch(baseUrl + `fine/${fine.id}`, {
        ...
        body: JSON.stringify({
          ...
          lateEntryFineAmount: Number(
            (fine.lateEntryFineAmount * fine.lateEntryAmount).toFixed(2) || 0
          ),
          lateEntryAmount: Number(fine.lateEntryAmount || 1),
          totalAmount: Number(
            (
              prevTotalAmount +
              fine.lateEntryFineAmount * fine.lateEntryAmount -
              prevLateEntryFineAmount
            ).toFixed(2) || 0
          ),
    ...
    try {
      const data = updateDetail.map((fine) => {
        const {
          ...
          fineDetail?.find(
            (value) =>
              value?.id === fine?.id && value?.staffId === fine?.staffId
          ) || {}; // Use fallback to avoid destructuring undefined
  ...
  useEffect(() => {
    setUpdateDetail([
      ...fineDetail.map((fine) => ({ id: fine.id, staffId: fine.staffId })),
    ]);
  }, [selectAllFine]);
  ...
  useEffect(() => {
    if (
      fineType === "Fixed Amount Per Minute" &&
      fixedFine > 0 &&
      selectAllFine === true
    ) {
      setStaffPerMinSalary([]);
      setStaffPerMinSalary(
        staffPerMinSalary?.map((fine) => ({
          staffId: fine?.staffId,
          finePerMinute: fixedFine,
        }))
      );
    ...
    if (fineType === "Half Day" && selectAllFine === true) {
      setUpdateDetail(
        fineDetail?.map((fine) => ({
          ...
          staffId: fine?.staffId,
          lateEntryFineAmount:
            staffPerMinSalary?.find((staff) => staff?.staffId === fine?.staffId)
              ?.finePerMinute *
            0.5 *
            8 *
            60, // Half-day fine logic
          lateEntryAmount: 1,
  ...
  return (
    <div className="w-full p-[20px]">
      <div className="">
        <Link className="text-[14px]" to="/attendence_summary">
          {" "}
      ...
      <div className="flex items-center gap-[10px] mb-[20px] ">
        ...
        <div className="relative inline-block text-left">
          <select
            ...
            className="select-pe rounded-md pl-[2px] text-[14px] p-[8px]"
          >
            {fixedAmountOptions.map((item, index) => (
          ...
          selectAllFine && (
            <div className="flex items-center gap-[10px]  rounded-md p-[6px] font-medium select-pe">
              <input
                ...
                id=""
                value={fixedFine}
                onChange={(e) => setFixedFine(e.target.value)}
              />
            </div>
          )}
        <div>
          <button
            className="p-2 pl-[20px] pr-[20px] border transition duration-200 bg-[#27004a] rounded-md text-[white] hover:bg-[#fff] hover:text-[#27004a] hover:border-[#27004a] hover:border"
            type="submit"
            onClick={updateMultipleStaffSameFine}
          >
            Apply
          </button>
        </div>
        ...
        return (
          <div className="shadow-md rounded-md p-[20px] mb-[20px]  ">
              ...
              <Select
                ...
                onChange={(selectedOptions) => {
                  setUpdateDetail((prev) => {
                    ...
                    return [
                      ...filteredPrev,
                      { ...exists, shiftId: updatedShifts[0] },
                    ];
                  });
                }}
                options={allshift}
            ...
            <div className="flex items-center gap-[16px] w-full">
              <input
                checked={updateDetail?.some(
                  (value) =>
                    value?.id === item?.id && value?.staffId === item?.staffId
                )}
                onClick={() => {
                  setUpdateDetail((prev) => {
                    ...
                    if (exists) {
                      ...
                      return [
                        ...prev,
                        { id: item?.id, staffId: item?.staffId },
                      ];
                    }
                  });
                }}
              ...
              <div>
                <h5 className="text-[14px]">
                  {staff?.find(
                    (staff) => staff?.staffDetails?.id === item?.staffId
                  )?.name || "N/A"}
                </h5>
                <p className="text-[12px] text-[#ad9f9f]">
                  In:{" "}
                  {item?.punchRecord?.punchIn?.punchInDate
                    ? new Date(
                        item.punchRecord.punchIn.punchInDate
                      ).toLocaleTimeString()
                    : "N/A"}{" "}
                  | Out:{" "}
                  {item?.punchRecord?.punchOut?.punchOutDate
                    ? new Date(
                        item.punchRecord.punchOut.punchOutDate
                      ).toLocaleTimeString()
                    : "N/A"}
                ...
                <div>
                  <h5 className="text-[12px] font-medium">
                    {item?.lateEntryFineHoursTime?.split(":")[0] !== "00"
                      ? `${item?.lateEntryFineHoursTime?.split(":")[0]} hr ${
                          item?.lateEntryFineHoursTime?.split(":")[1]
                        } min late`
                      : `${
                          item?.lateEntryFineHoursTime?.split(":")[1]
                        } min late`}
                  ...
                  <div>
                      ...
                      onChange={(value) => {
                        ...
                        setUpdateDetail((prev) => {
                          ...
                          return [
                            ...
                            {
                              ...exists,
                              lateEntryFineHoursTime: value?.format("HH:mm"),
                              lateEntryFineAmount:
                                staffPerMinSalary?.find(
                                  (perMinSalary) =>
                                    perMinSalary?.staffId === item?.staffId
                                )?.finePerMinute * totalMinutes,
                            },
                          ];
                        });
                      }}
                    ...
                    <p className="text-[12px] mt-[8px]">
                      ...
                      {(() => {
                        ...
                        const fineAmount =
                          detail?.lateOutOvertimeAmount ||
                          item?.lateOutOvertimeAmount ||
                          0;
                        if (
                          overTimeType === "Fixed Amount" &&
                          fixedOverTime > 0
                        )
                          return fixedFine * entryAmount || 0;
                        ...
                        value={
                          updateDetail?.find(
                            (value) =>
                              value?.id === item?.id &&
                              value?.staffId === item?.staffId
                          )?.lateEntryAmount || item?.lateEntryAmount
                        }
                        disabled={
                          !updateDetail?.some(
                            (value) =>
                        ...
                        onChange={(e) => {
                          setUpdateDetail((prev) => {
                            ...
                            return [
                              ...
                              {
                                ...exists,
                                lateEntryAmount: Number(e.target.value),
                              },
                            ];
                          });
                        }}
